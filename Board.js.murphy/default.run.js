...
if(Board({id:"5a938de4e0c2896bd94c7434"}).moveAllCards({from:"Test from",to:"Test to"}).length < 3)
    console.log("Unexpected number of elements returned during move all cards operation");

if(Board({id:"5a938de4e0c2896bd94c7434"}).list({name: "Test from"}).name() != 'Test from')
    console.log("Got back incorrect list from Board.list with string");

if(Board({id:"5a938de4e0c2896bd94c7434"}).list({name: new RegExp("Test .+")}).name() != 'Test from')
    console.log("Got back incorrect list from Board.list with RegExp");

if(Board({id:"5a938de4e0c2896bd94c7434"}).lists({name: "Test from"}).length() != 1)
    console.log("Got back incorrect list from Board.lists with string");

if(Board({id:"5a938de4e0c2896bd94c7434"}).lists({name: new RegExp("Test .+")}).length() != 2)
